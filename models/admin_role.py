...
class AdminAction(Base):
    ...
    admin_id = Column(BigInteger, nullable=False, index=True)  # telegram_id of admin who requested
    action_type = Column(String(50), nullable=False)  # 'free_access', 'broadcast', 'ban', 'unban'
    target_user_id = Column(BigInteger, nullable=True)  # For user-specific actions
    details = Column(Text, nullable=True)  # JSON string with action details
    status = Column(String(20), default='pending')  # 'pending', 'approved', 'rejected'
    created_at = Column(DateTime, default=datetime.now)
    reviewed_at = Column(DateTime, nullable=True)
    reviewed_by = Column(BigInteger, nullable=True)  # telegram_id of super admin who reviewed
